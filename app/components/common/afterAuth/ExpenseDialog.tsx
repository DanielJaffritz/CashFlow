...
const ExpenseDialog = ({ isOpen, setIsOpen }: DialogProps) => {
  ...
  return (
    <dialog open closedby="any" onClose={() => setIsOpen(false)} className='transition-all flex flex-col w-3/4 md:w-1/2  h-2/3 md:h-5/6 shadow-2xl rounded-2xl fixed top-[50%] left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] dark:bg-zinc-800'>
      <div className="shadow flex flex-row justify-between rounded-t-2xl p-6">
        <h1 className="text-2xl font-semibold dark:text-white">Add New Expense</h1>
        ...
        <form onSubmit={HandleSubmit} className="flex flex-col justify-center items-center">
          ...
          <div className="flex flex-row justify-between px-5 w-full">
            <div className="flex flex-col w-35 md:w-full">
              ...
              <select defaultValue='' id="category" name="category" required className=' outline-zinc-400 rounded-md p-3 mr-1 md:mr-5 border border-zinc-200 bg-bg-app dark:bg-zinc-700 dark:border-transparent dark:text-white'>
            ...
            <div className="flex flex-col w-35 md:w-full">
              <label>Date</label>
              <input id="date" name="date" type="date" required className="p-3 appearance-none outline-zinc-400 rounded-md border border-zinc-200 bg-bg-app dark:bg-zinc-700 dark:text-white dark:border-transparent"></input>
            </div>
          </div>
          <div className="w-full flex flex-col px-5">
            <p className="text-white">Description</p>
            <textarea id='description' name="description" rows={4} maxLength={110} placeholder="Description of your expense" required className="resize-none w-full outline-zinc-400 border border-zinc-200 bg-bg-app dark:bg-zinc-700 dark:text-white dark:border-transparent"></textarea>
          </div>
          <div className="flex flex-row justify-between p-5">
            ...
            <button className="bg-amber-400 hover:bg-amber-500 w-full p-3 rounded-md cursor-pointer" type="submit">Save expense</button>
          </div>
        </form>
      </div>
    </dialog>
